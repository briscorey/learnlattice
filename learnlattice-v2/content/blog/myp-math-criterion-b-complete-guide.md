...
## The Four Strands of Criterion B

At MYP Year 3 and above, Criterion B assesses four connected skills:
...
4. **Prove, or verify and justify** these general rules

The progression from strand to strand maps directly onto the achievement levels. A student who can only identify patterns (strand 1-2) demonstrates levels 1-4. A student who can describe and verify (strands 1-3) demonstrates levels 5-6. A student who can prove or justify (all four strands) demonstrates levels 7-8.
...
**Justification** means connecting the rule to the structure of the pattern: "Each new shape adds 3 dots because it extends by one triangle on each of three sides. The +1 accounts for the central dot that every shape shares." This explains *why* the rule must work for all cases.
...
**4. Multiple levels of generalisation.** A great investigation has a rule that works for the cases shown AND invites extension: "What if the starting shape were different? What if we used hexagons instead of triangles?" This provides the high ceiling.

For detailed guidance with step-by-step examples, read our blog post: [How to Write a Criterion B Investigation That Students Actually Want to Do](/blog/how-to-write-a-criterion-b-investigation/).

## Common Criterion B Assessment Mistakes

**Mistake 1: Scaffolding too heavily.** If the task tells students exactly what to investigate, gives them a table to fill in, and tells them to find the nth term rule, they cannot demonstrate the "select and apply" strand at a high level. The best tasks provide an entry point but leave the direction of investigation open.

**Mistake 2: Confusing description with justification.** A student who writes "the rule is 2n + 1" has described the pattern. A student who explains why each new case adds 2 by referring to the geometric structure has justified it. These are different achievement levels, and the task-specific clarification should distinguish them clearly.

**Mistake 3: Using only linear patterns.** If every investigation in your programme has a rule of the form an + b, students learn a mechanical process rather than genuine investigation. Include quadratic patterns (n², triangular numbers), geometric patterns (doubling), and patterns that require different representations.

**Mistake 4: Marking based on the final rule, not the process.** A student who guesses the correct formula by trial and error has not demonstrated the same level as a student who derives it logically. Criterion B assesses the *investigation process*, not just the *answer*.

For a deeper dive: [Five Criterion B Mistakes Teachers Make (and How to Fix Them)](/blog/five-criterion-b-mistakes/).

## Criterion B Assessment Packs with Annotated Exemplars
...
Peter Liljedahl's research on Building Thinking Classrooms aligns naturally with Criterion B. His emphasis on starting with thinking tasks (not review), using vertical non-permanent surfaces for visible reasoning, and random grouping to prevent passive copying all support the kind of genuine mathematical investigation that Criterion B demands. Read more: [Thinking Classrooms Meet MYP Mathematics](/blog/thinking-classrooms-meet-myp/).
